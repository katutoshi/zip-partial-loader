...
//! 代わりに、公開 API の `LSZR::new` に不正データを渡して Err が返ることを
//! 実際に呼び出しで検証する形にする。
...
#[wasm_bindgen_test]
fn lszr_new_rejects_short_data() {
    // 22 バイト未満は EOCD 最小長 (ZIP 仕様) を満たさないので LSZR::new は Err を返すのが
    // 期待挙動。wasm32 ターゲットで LSZR の Rust 側 API がリンクし呼べることも同時に確認する。
    let short: Vec<u8> = vec![0u8; 10];
    let result = LSZR::new(short);
